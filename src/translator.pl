...
constrain_args(X, _, X, []) :- (var(X); atomic(X)), !.
constrain_args([F, A, B], _, [A|B], []) :- F == cons, !.
constrain_args([F|Args], Ctxt, Var, Goals) :- atom(F),
                                        fun(F), !,
                                        translate_expr([F|Args], Ctxt, GoalsExpr, Var),
                                        flatten(GoalsExpr, Goals).
constrain_args(In, Ctxt, Out, Goals) :- maplist(constrain_args_c(Ctxt), In, Out, NestedGoalsList),
                                  flatten(NestedGoalsList, Goals), !.
constrain_args_c(Ctxt, In, Out, Goals) :- constrain_args(In, Ctxt, Out, Goals).
...
translate_clause(Input, Ctxt, (Head :- BodyConj)) :- Input = [=, [F|Args0], BodyExpr],
                                               ...
                                               translate_expr(BodyExpr, Ctxt, GoalsBody, ExpOut),
...
translate_expr(X, _, [], X)          :- (var(X) ; atomic(X)), !.
translate_expr([H0|T0], Ctxt, Goals, Out) :-
        safe_rewrite_streamops([H0|T0],[H|T]),
        translate_expr(H, Ctxt, GsH, HV),
        %--- Non-determinism ---:
        ( HV == superpose, T = [Args], is_list(Args) -> build_superpose_branches(Args, Ctxt, Out, Branches),
                                                        disj_list(Branches, Disj),
                                                        append(GsH, [Disj], Goals)
        ; HV == collapse, T = [E] -> translate_expr_to_conj(E, Ctxt, Conj, EV),
                                     append(GsH, [findall(EV, Conj, Out)], Goals)
        ; HV == cut, T = [] -> append(GsH, [(!)], Goals),
                               Out = true
        ; HV == test, T = [Expr, Expected] -> translate_expr_to_conj(Expr, Ctxt, Conj, Val),
                                              translate_expr(Expected, Ctxt, GsE, ExpVal),
        ...
        ; HV == once, T = [X] -> translate_expr_to_conj(X, Ctxt, Conj, Out),
                                 append(GsH, [once(Conj)], Goals)
        ; HV == hyperpose, T = [L] -> build_hyperpose_branches(L, Ctxt, Branches),
                                      append(GsH, [concurrent_and(member((Goal,Res), Branches), (call(Goal), Out = Res))], Goals)
        %--- Sequential execution ---:
        ; HV == progn, T = Exprs -> translate_args(Exprs, Ctxt, GsList, Outs),
                                    append(GsH, GsList, Tmp),
                                    last(Outs, Out),
                                    Goals = Tmp
        ; HV == prog1, T = Exprs -> Exprs = [First|Rest],
                                    translate_expr(First, Ctxt, GsF, Out),
                                    translate_args(Rest, Ctxt, GsRest, _),
                                    append(GsH, GsF, Tmp1),
                                    append(Tmp1, GsRest, Goals)
        %--- Conditionals ---:
        ; HV == if, T = [Cond, Then, Else] -> translate_expr_to_conj(Cond, Ctxt, ConC, Cv),
                                              translate_expr_to_conj(Then, Ctxt, ConT, Tv),
                                              translate_expr_to_conj(Else, Ctxt, ConE, Ev),
        ...
        ; HV == case, T = [KeyExpr, PairsExpr] -> ( select(Found0, PairsExpr, Rest0),
                                                    ...
                                                    NormalCases = Rest0
                                                    -> translate_expr_to_conj(KeyExpr, Ctxt, GkConj, Kv),
                                                       translate_case(NormalCases, Ctxt, Kv, Out, CaseGoal, KeyGoal),
                                                       translate_expr_to_conj(DefaultExpr, Ctxt, ConD, DOut),
                                                     ...
                                                     ; translate_expr(KeyExpr, Ctxt, Gk, Kv),
                                                       translate_case(PairsExpr, Ctxt, Kv, Out, IfGoal, KeyGoal),
                                                       append([GsH, Gk, KeyGoal, [IfGoal]], Goals) )
        %--- Unification constructs ---:
        ; (HV == let ; HV == chain), T = [Pat, Val, In] -> translate_expr(Pat, Ctxt, Gp, Pv),
                                                           constrain_args(Pv, Ctxt, P, Gc),
                                                           translate_expr(Val, Ctxt, Gv, V),
                                                           translate_expr(In,  Ctxt, Gi, Out),
                                                           append([GsH,[(P=V)],Gp,Gv,Gi,Gc], Goals)
        ; HV == 'let*', T = [Binds, Body] -> letstar_to_rec_let(Binds,Body,RecLet),
                                             translate_expr(RecLet, Ctxt, Goals, Out)
        ; HV == sealed, T = [Vars, Expr] -> translate_expr_to_conj(Expr, Ctxt, Con, Out),
                                            Goals = [copy_term(Vars,Con,_,Ncon),Ncon]
        %--- Iterating over non-deterministic generators without reification ---:
        ; HV == 'forall', T = [GF, TF]
          -> ( is_list(GF) -> GF = [GFH|GFA],
                              translate_expr(GFH, Ctxt, GsGFH, GFHV),
                              translate_args(GFA, Ctxt, GsGFA, GFAv),
                              append(GsGFH, GsGFA, GsGF),
                              GenList = [GFHV|GFAv]
                            ; translate_expr(GF, Ctxt, GsGF, GFHV),
                              GenList = [GFHV] ),
        ...
        ; HV == 'foldl-atom', T = [List, Init, AccVar, XVar, Body]
          -> translate_expr_to_conj(List, Ctxt, ConjList, L),
             translate_expr_to_conj(Init, Ctxt, ConjInit, InitV),
             translate_expr_to_conj(Body, Ctxt, BodyConj, BG),
        ...
        ; HV == 'map-atom', T = [List, XVar, Body]
          -> translate_expr_to_conj(List, Ctxt, ConjList, L),
             translate_expr_to_conj(Body, Ctxt, BodyCallConj, BodyCall),
        ...
        ; HV == 'filter-atom', T = [List, XVar, Cond]
          -> translate_expr_to_conj(List, Ctxt, ConjList, L),
             translate_expr_to_conj(Cond, Ctxt, CondConj, CondGoal),
             exclude(==(true), [ConjList], CleanConjs),
             append(GsH, CleanConjs, GsMid),
        ...
        ; HV == '|->', T = [Args, Body] -> next_lambda_name(F),
                                           ...
                                           translate_clause([=, [F|FullArgs], Body], Ctxt, Clause),
        ...
        ; HV == match, T = [Space, Pattern, Body] -> translate_expr(Space, Ctxt, G1, S),
                                                     translate_expr(Body, Ctxt, GsB, Out),
                                                     append(G1, [match(S, Pattern, Out, Out)], G2),
                                                     append(G2, GsB, Goals)
        %--- Manual dispatch options: ---
        ; HV == 'import!', T = [Space, File] ->
            translate_expr(Space, Ctxt, GsS, Sv),
            translate_expr(File, Ctxt, GsF, Fv),
            append(GsS, GsF, GsArgs),
            Goal = 'import!'(Sv, Fv, Ctxt, Out),
        ...
        ; HV == quote, T = [Expr] -> append(GsH, [], Inner),
                                     ...
                                     Goals = Inner
        ; HV == 'catch', T = [Expr] ->
          translate_expr(Expr, Ctxt, GsExpr, ExprOut),
          append(GsH, [], Inner),
        ...
        ; translate_args(T, Ctxt, GsT, AVs),
          ...
          ( is_list(AVs),
            ...
            -> ( catch(match('&self', [':', Fun, TypeChain], TypeChain, TypeChain), _, fail)
                 -> TypeChain = [->|Xs],
                    ...
                    translate_args_by_type(T, Ctxt, ArgTypes, GsT2, AVsTmp0),
          ...
          ; is_list(HV) -> eval_data_term(HV, Ctxt, Gd, HV1),
...
translate_args_by_type([A|As], Ctxt, [T|Ts], GsOut, [AV|AVs]) :-
                      ( T == 'Expression' -> AV = A, GsA = []
                                           ; translate_expr(A, Ctxt, GsA1, AV),
                                             ( (T == '%Undefined%' ; T == 'Atom')
                                               -> GsA = GsA1
                                                ; append(GsA1, [('get-type'(AV, T) *-> true ; 'get-metatype'(AV, T))], GsA))),
                                             translate_args_by_type(As, Ctxt, Ts, GsRest, AVs),
                                             append(GsA, GsRest, GsOut).

%Handle data list:
eval_data_term(X, _, [], X) :- (var(X); atomic(X)), !.
eval_data_term([F|As], Ctxt, Goals, Val) :- ( atom(F), fun(F) -> translate_expr([F|As], Ctxt, Goals, Val)
                                                         ; eval_data_list([F|As], Ctxt, Goals, Val) ).

%Handle data list entry:
eval_data_list([], _, [], []).
eval_data_list([E|Es], Ctxt, Goals, [V|Vs]) :- ( is_list(E) -> eval_data_term(E, Ctxt, G1, V) ; V = E, G1 = [] ),
                                         eval_data_list(Es, Ctxt, G2, Vs),
                                         append(G1, G2, Goals).
...
translate_case([[K,VExpr]|Rs], Ctxt, Kv, Out, Goal, KGo) :- translate_expr_to_conj(VExpr, Ctxt, ConV, VOut),
                                                      constrain_args(K, Ctxt, Kc, Gc),
                                                      build_branch(ConV, VOut, Out, Then),
                                                      ( Rs == [] -> Goal = ((Kv = Kc) -> Then)
                                                                  ; translate_case(Rs, Ctxt, Kv, Out, Next, KGi),
                                                                    Goal = ((Kv = Kc) -> Then ; Next) ),
                                                      append([Gc,KGi], KGo).

%Translate arguments recursively:
translate_args([], _, [], []).
translate_args([X|Xs], Ctxt, Goals, [V|Vs]) :- translate_expr(X, Ctxt, G1, V),
                                         translate_args(Xs, Ctxt, G2, Vs),
                                         append(G1, G2, Goals).
...
build_superpose_branches([], _, _, []).
build_superpose_branches([E|Es], Ctxt, Out, [B|Bs]) :- translate_expr_to_conj(E, Ctxt, Conj, Val),
                                                 build_branch(Conj, Val, Out, B),
                                                 build_superpose_branches(Es, Ctxt, Out, Bs).

%Build hyperpose branch as a goal list for concurrent_maplist to consume:
build_hyperpose_branches([], _, []).
build_hyperpose_branches([E|Es], Ctxt, [(Goal, Res)|Bs]) :- translate_expr_to_conj(E, Ctxt, Goal, Res),
                                                      build_hyperpose_branches(Es, Ctxt, Bs).
